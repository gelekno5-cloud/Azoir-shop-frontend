...
Each section reads `data-animation` and gets a different entrance. Sections with `data-persist="true"` stay visible once animated in.
...
- **FRAME_SPEED < 1.8** — product animation feels sluggish
- **Hero < 20% scroll range** — first impression needs breathing room
- **Same animation for consecutive sections** — never repeat back-to-back
- **Scroll height < 800vh** for 6 sections — everything feels rushed
- **Pure cover mode at 1.0** — product clips into header, use IMAGE_SCALE 0.82-0.90
